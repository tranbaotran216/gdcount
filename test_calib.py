...
def _scores_from_outputs(outputs: Dict[str, Any], prefer_calib: bool = True) -> torch.Tensor:
    """
    Return scores (B,Q):
      - prefer calib_scores nếu có (NEW)
      - fallback: sigmoid(max_token_logit) từ pred_logits như cũ
    """
    if prefer_calib and ("calib_scores" in outputs) and isinstance(outputs["calib_scores"], torch.Tensor):
        s = outputs["calib_scores"]
        if s.dim() == 2:
            return s.float()
        # an toàn
        return s.view(s.shape[0], -1).float()
...
def count_by_det_nms(
    outputs: Dict[str, Any],
    threshold: float,
    nms_iou: float,
    prefer_calib: bool = True,
) -> torch.Tensor:
    """
    Return pred_count (B,) from scores + pred_boxes with threshold + NMS.
    Scores ưu tiên calib_scores nếu có (NEW).
    Uses normalized xyxy (0..1), NMS vẫn hợp lệ.
    """
    scores = _scores_from_outputs(outputs, prefer_calib=prefer_calib)  # (B,Q)
...
def build_exemplar_inputs(batch: Dict[str, Any], device: str):
    # dataset của bạn đặt exemplar ở meta["exemplar_xyxy"] (K,4) theo pixel
...
def parse_args() -> argparse.Namespace:
    ...
    # ---- Use calib for counting ----
    parser.add_argument("--prefer-calib", action="store_true", default=True,
                        help="Ưu tiên dùng outputs['calib_scores'] để count nếu có.")
    parser.add_argument("--no-prefer-calib", dest="prefer_calib", action="store_false")

    # ---- Exemplar (optional) ----
    parser.add_argument("--use-exemplar", action="store_true", help="Test text+exemplar (nếu model hỗ trợ).")
...
# thêm loss_calib để log nếu có
LOSS_KEYS = ["loss_total", "count_mae", "loss_ce", "loss_bbox", "loss_giou", "loss_query", "loss_calib"]
...
def evaluate(
    ...
    pbar = tqdm(enumerate(loader), total=len(loader), desc="Test [test]", ncols=120)
    ...
    with torch.no_grad():
        for _, batch in pbar:
            ...
            pred_counts = count_by_det_nms(
                ...
                prefer_calib=args.prefer_calib,
            ).to(gt_counts.dtype)
...
def load_model_checkpoint(model_ckpt_path: str, model: torch.nn.Module, device: str) -> Dict[str, Any]:
    ...
    # IMPORTANT: ckpt cũ có thể chưa có calibrator/ex_mod -> strict=False
...
def init_test_csv(log_dir: str, filename: str) -> str:
    ...
    if not os.path.exists(path):
        with open(path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow([
                "model_ckpt", "ckpt_epoch", "split", "batch_size",
                "threshold", "nms_iou", "prefer_calib",
                "loss_total", "count_mae", "loss_ce", "loss_bbox", "loss_giou", "loss_query", "loss_calib",
                "mae", "rmse",
            ])
    return path
...
def main():
    ...
    # Test: thường chỉ cần log, không cần query/calib loss -> để False.
    # Nhưng nếu bạn muốn log loss_calib, bật use_calib_loss=True và lambda_calib>0.
    criterion = MultiTaskLoss(
        ...
        nms_iou=args.nms_iou,
        lambda_calib=1.0,
        use_calib_loss=False,   # <-- đổi True nếu muốn log loss_calib
    )
