...
from scripts.losses import MultiTaskLoss   # multi-task loss
...
def count_by_det_nms(outputs: Dict[str, Any], threshold: float, nms_iou: float) -> torch.Tensor:
    ...
    # no boxes or no torchvision nms => fallback threshold count
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path tới file config GroundingDINO (GroundingDINO_SwinT_OGC.py)",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
        help="Path tới weights .pth GroundingDINO pretrain",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path tới annotation_FSC147_384.json",
    )
    parser.add_argument(
        "--img-root",
        type=str,
        required=True,
        help="Thư mục chứa ảnh resize (images_384_VarV2)",
    )
    parser.add_argument(
        "--split-file",
        type=str,
        required=True,
        help="File JSON Train_Test_Val_FSC_147.json",
    )
    parser.add_argument(
        "--class-map",
        type=str,
        required=True,
        help="File ImageClasses_FSC147.txt",
    )
    ...
    parser.add_argument("--nms-iou", type=float, default=0.5, help="IoU threshold cho NMS khi đếm")
    parser.add_argument("--amp", action="store_true", default=True, help="Bật AMP (autocast + GradScaler)")
    parser.add_argument("--use-exemplar", action="store_true", help="Train text + exemplar (FSC147 exemplar_boxes)")
    parser.add_argument("--exp-name", type=str, default="", help="Tên experiment để tách log/ckpt (vd: text_exemplar)")
    # ---- GDCount config ----
    parser.add_argument(
        "--threshold",
        type=float,
        default=0.23,
        help="Ngưỡng hard count trên logit query",
    )
    parser.add_argument(
        "--soa-level",
        type=int,
        default=-1,
        help="Index feature level dùng SOA (0..n-1, âm tính từ cuối)",
    )
    parser.add_argument(
        "--freeze-keywords",
        type=str,
        ...
        help="Các từ khoá để freeze param GroundingDINO",
    )
    parser.add_argument(
        "--resume",
        type=str,
        default="",
        help="Path checkpoint GDCount (.pth) để train tiếp (resume).",
    )
    parser.add_argument(
        "--start-epoch",
        type=int,
        default=-1,
        help="Epoch bắt đầu (override). Nếu = -1 thì lấy theo checkpoint (epoch+1).",
    )
...
def create_dataloader(
    ...
    dataset = FSC147Dataset(
        ...
        normalize=True,
        density_root=None,    # hiện tại chưa dùng GT density
    )
    loader = DataLoader(
        dataset,
...
def create_model(args: argparse.Namespace) -> torch.nn.Module:
    cfg = GDCountConfig(
        ...
        feature_dim=256,  # hidden_dim của SwinT GroundingDINO
...
def create_optimizer(
    ...
    """
    Nhóm param:
      - SOA + CountHead: lr lớn hơn
      - phần còn lại: lr nhỏ hơn
    """
    head_params = []
    base_params = []
...
def build_targets(batch: Dict[str, Any], device: str):
    ...
    for i in range(B):
        ...
        # tạo pseudo boxes từ points (cách bạn đang làm ok)
        ...
        targets.append({
            "boxes": boxes,
            "labels": labels,
            "count": gt_counts[i].view(()),  # <-- quan trọng để loss_count đúng
        })
...
def train_one_epoch(
    ...
    for step, batch in pbar:
        ...
        with autocast(enabled=(args.amp and device.startswith("cuda"))):
            ...
            outputs["img_hw"] = (images.shape[-2], images.shape[-1]) # for debug
...
def evaluate(
    ...
    with torch.no_grad():
        for step, batch in pbar:
            ...
            # metrics: MAE / RMSE theo Counting-by-Detection (threshold + NMS)
...
def load_checkpoint(
    ...
    # epoch trong ckpt là epoch đã hoàn thành
...
def main():
    ...
    criterion_detect = build_criterion_detect(
        ...
        num_classes=1,      # prompt 1 class -> binary objectness vẫn OK
        class_cost=1.0,
        bbox_cost=5.0,
        giou_cost=2.0,
        lambda_cls=1.0,
    ...
    # CSV logger
    csv_path = init_csv_logger(log_dir="logs", filename="train_log.csv")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Best checkpoint (theo val total loss)
